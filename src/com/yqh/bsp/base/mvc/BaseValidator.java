...
public abstract class BaseValidator implements Interceptor {
	...
	protected void handleError(Controller c) {
		c.renderText(JsonKit.toCompatibleJSONString(baseResponse));
	}
	...
	protected void addError(String errorKey, String errorMessage) {
		invalid = true;
		baseResponse.setCode(Integer.parseInt(errorKey));
		baseResponse.setErrmsg(errorMessage);
		if (shortCircuit) {
	...
	protected void validateMobile(String field, String errorKey, String errorMessage) {
		String value = controller.getPara(field);
        if (!RegularUtil.isPhoneNumber(value))
        	addError(errorKey, errorMessage);
	}
	
	protected void validateResubmit(String field,String errorKey, String errorMessage ) {
		String value = controller.getPara(field);
		String userId = controller.getPara("user_id");
		String cavalue = CacheKit.get("cachehalfhour", "urlsign");
		if ((userId+value).equals(cavalue)) {
			CacheKit.remove("cachehalfhour", "urlsign");
			addError(errorKey, errorMessage);
		} else {
			CacheKit.put("cachehalfhour", "urlsign", userId+value);
		}
	}
